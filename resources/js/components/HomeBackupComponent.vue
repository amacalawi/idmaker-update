                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div v-bind:class="{hidden: isnotQRcode}" class="camera-layer2 text-center primary-bg primary-border">
                                    ...
                                    <button :disabled="isDisabled == 1 || (isSearch == false && mobile_number.length < 11) || (isSearch == false && mobile_number == '')" @click="requestOTP" id="request-otp-btn" ref="request-otp-btn" type="button" class="btn btn-md secondary-border secondary-bg">
                                        request an OTP
                                    </button>
    ...
    export default {
        ...
        methods: {          
                ...
                // } else {
                    if (self.isQRcode == true) {
                        if (self.id_number !== '' && self.otp_1 !== '' && self.otp_2 !== '' && self.otp_3 !== '' && self.otp_4 !== '' && self.otp_5 !== '') { 
                            ...
                            console.log(base_url + 'scan-otp?type=staff&id='+ self.id_number +'&otp=' + otp + '&action=' + self.action)
                            axios.get(base_url + 'scan-otp?type=staff&id='+ self.id_number +'&otp=' + otp + '&action=' + self.action)
                                ...
                                if (res.data.type == 'success') {
                                    ...
                                    res.data.data.forEach((info) => { 
                                        ...
                                        if (info.identification_no) {
                                            var image = new Image()
                                            image.src = base_url + 'images/staffs/' + info.identification_no + '.jpg'
                                            if (image.width == 0) {
                                                self.imgSource = base_url + 'images/staffs/' + info.identification_no + '.JPG'
                                            } else {
                                                self.imgSource = base_url + 'images/staffs/' + info.identification_no + '.jpg'
                                            }
                                        }
                ...
                // } else {
                    if (self.isQRcode == true) {
                        if (self.id_number !== '' && self.otp_1 !== '' && self.otp_2 !== '' && self.otp_3 !== '' && self.otp_4 !== '' && self.otp_5 !== '') { 
                            self.action = 'signout'
                            let otp = self.otp_1 + '' + self.otp_2 + '' + self.otp_3  + '' + self.otp_4  + '' + self.otp_5 
                            console.log(base_url + 'scan-otp?type=staff&id='+ self.id_number +'&otp=' + otp + '&action=' + self.action + '&subject=' + self.subject)
                            axios.get(base_url + 'scan-otp?type=staff&id='+ self.id_number +'&otp=' + otp + '&action=' + self.action + '&subject=' + self.subject)
                            .then(function (response) {
                                // handle success
                                var res = response  
                                console.log(response)
            ...
            searchMethod() {
                let self = this
                if (this.id_number !== '') { 
                    console.log(base_url + 'search?type=staff&id_number='+ self.id_number)
                    axios.get(base_url + 'search?type=staff&id_number='+ self.id_number)
                    .then(function (response) {
                        // handle success
                        var res = response  
                        console.log(response)
            ...
            requestOTP() {
                let self = this
                if (this.id_number !== '') { 
                    console.log(base_url + 'request-otp?type=staff&id_number='+ self.id_number + '&mobile_number=' + self.mobile_number)
                    axios.get(base_url + 'request-otp?type=staff&id_number='+ self.id_number + '&mobile_number=' + self.mobile_number)
                    .then(function (response) {
                        // handle success
                        var res = response  
                        console.log(response)
            ...
            onDecode (result) {
                let self = this
                this.result = result
                console.log(base_url + 'scan?type=staff&id='+ result +'&action=' + self.action);
                axios.get(base_url + 'scan?type=staff&id='+ result +'&action=' + self.action)
                .then(function (response) {
                    // handle success
                    var res = response  
                    console.log(response)
